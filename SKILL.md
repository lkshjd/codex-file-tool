...
## 含特殊字符的替换：--find-file + --with-file

替换内容含 `$` `{` `}` 时，**MUST** 走临时文件中转。
固定目录：`%USERPROFILE%\.codex\tmp\`

**完整调用流程：**

```javascript
// Step 1-2: mcp__node_repl__js 写临时文件到固定目录
var fs = await import("node:fs");
var tmp = require("node:os").homedir() + "/.codex/tmp";
...
node _tool.js replace "src/app.dart" --find-file "%USERPROFILE%\.codex\tmp\find.txt" --with-file "%USERPROFILE%\.codex\tmp\replace.txt"
```
...
(replace-safe 是上述流程的简写别名，任选一种均可)
